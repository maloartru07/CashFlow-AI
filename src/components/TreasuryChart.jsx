...
export default function TreasuryChart({ results }) {
  const data = results.map(r => ({
    ...
    tension: r.enTension ? r.soldeFin : null
  }))

  const hasTension = results.some(r => r.enTension)
  const allSoldes = results.map(r => r.soldeFin)
  const minSolde = Math.min(...allSoldes)
  const maxSolde = Math.max(...allSoldes)
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            {hasTension && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#fef2f2', padding: '6px 12px', borderRadius: '8px' }}>
                ...
                <span style={{ fontSize: '12px', color: '#dc2626', fontWeight: '600' }}>Zone de tension détectée</span>
              </div>
            )}
      ...
      <div style={{ padding: '24px 12px 16px' }}>
        <ResponsiveContainer width="100%" height={340}>
          <ComposedChart data={data} margin={{ top: 8, right: 24, left: 12, bottom: 8 }}>
            ...
            <YAxis tickFormatter={v => v >= 1000 || v <= -1000 ? `${(v/1000).toFixed(0)}k€` : `${v}€`}
              tick={{ fill: '#64748b', fontSize: 11 }} axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} />
            <Legend wrapperStyle={{ paddingTop: '16px', fontSize: '12px' }} />
            <ReferenceLine y={0} stroke="#dc2626" strokeWidth={1.5} strokeDasharray="4 4" />
            <Bar dataKey="Entrées" fill="#bfdbfe" radius={[3,3,0,0]} opacity={0.7} />
            <Bar dataKey="Sorties" fill="#fecaca" radius={[3,3,0,0]} opacity={0.7} />
            <Area type="monotone" dataKey="Solde fin de mois"
              stroke="#2563a8" strokeWidth={2.5} fill="url(#areaGreen)"
              dot={(props) => {
                const { cx, cy, payload } = props
                const isTension = payload['Solde fin de mois'] < 0
                return <circle key={cx} cx={cx} cy={cy} r={5} fill={isTension ? '#dc2626' : '#2563a8'} stroke="white" strokeWidth={2} />
              }}
            />
          </ComposedChart>
        </ResponsiveContainer>
