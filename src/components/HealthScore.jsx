...
export default function HealthScore({ score }) {
  ...
  const endAngle = -20
  const totalAngle = 360 - startAngle + endAngle // = 140 degrees effectively: from 200 to 340
  // Actually: arc from 220deg to -40deg going clockwise = 240 degrees total
  // Let's do it simply: semicircle gauge
  const toRad = (deg) => (deg * Math.PI) / 180
  const circumference = 2 * Math.PI * radius
  const arcDegrees = 240 // total sweep
