...
const KPI = ({ label, value, sub, color, icon }) => (
  <div style={{
    ...
    borderLeft: `4px solid ${color}`, boxShadow: '0 2px 12px rgba(15,35,64,0.06)'
  }}>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', alignContent: 'start' }}>
      ...
      <KPI label="Mois en tension" value={moisEnTension}
        sub={moisEnTension === 0 ? 'Aucune tension' : `${moisEnTension} mois concernés`}
      ...
      <KPI label="Couverture trésorerie" value={`${Math.max(0, couvertureJours)}j`}
        sub="Jours de charges couverts"
        color={couvertureJours < 30 ? '#ea580c' : '#2563a8'} icon="🛡️" />
      <KPI label="Flux moyen mensuel"
        value={formatCurrency(0)}
        sub="Voir graphique" color="#2563a8" icon="📊" />
    </div>
  )
}
