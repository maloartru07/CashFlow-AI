...
export default function InputForm({ initialData, onAnalyze }) {
  const [step, setStep] = useState(0) // 0 = infos générales, 1-12 = mois
        ...
        <div style={{ padding: '32px' }}>
          {step === 0 ? (
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {[3,6,9,12].map(n => (
                    <button key={n} type="button"
                      onClick={() => setData(d => ({ ...d, months: MONTH_NAMES.slice(0, n).map((m, i) => d.months[i] || { mois: m, encaissements: '', decaissements: '', echeancesFournisseurs: '', echeancesClients: '', chargesFixes: '' }) }))}
                      style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              ...
              {/* Quick fill for all months */}
              {monthIdx === 0 && (
                <div style={{ gridColumn: '1 / -1', marginTop: '8px', padding: '16px', background: '#f0f9ff', borderRadius: '10px', border: '1px solid #bae6fd' }}>
                  <p style={{ fontSize: '13px', color: '#0369a1', fontWeight: '600', marginBottom: '8px' }}>Astuce : valeurs identiques pour tous les mois</p>
                  <p style={{ fontSize: '12px', color: '#0284c7' }}>Renseignez les montants ci-dessus, puis cliquez « Appliquer à tous les mois » pour accélérer la saisie.</p>
                  <button type="button" onClick={() => {
                    const template = data.months[0]
                    setData(d => ({
                      ...d,
                      months: d.months.map((m, i) => i === 0 ? m : { ...template, mois: m.mois })
                    }))
                  }} style={{
                    marginTop: '10px', padding: '7px 16px', background: '#0369a1', color: 'white',
        ...
        <div style={{
          ...
          <button type="button" onClick={() => setStep(s => Math.max(0, s - 1))}
            ...
            style={{
              ...
              fontSize: '14px', fontWeight: '500', background: step === 0 ? '#f8fafc' : 'white',
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            {Array.from({ length: Math.min(totalSteps, 13) }).map((_, i) => (
              <div key={i} style={{
                width: i === step ? '20px' : '8px', height: '8px', borderRadius: '99px', transition: 'width 0.3s',
                background: i <= step ? '#2563a8' : '#e2e8f0'
              }} />
            ))}
          </div>
          ...
          {data.months.map((m, i) => (
            <button key={i} type="button" onClick={() => setStep(i + 1)}
              style={{
                ...
                border: '1px solid',
                borderColor: i + 1 === step ? '#2563a8' : '#e2e8f0',
