...
function getTensionRecommendation(result) {
  if (result.critique) return 'Activation immédiate d’une ligne de crédit recommandée. Déficit critique détecté.'
  if (result.totalSorties > result.totalEntrees * 1.3) return 'Décaler les paiements fournisseurs non urgents et relancer les impayés clients.'
  return 'Négocier un délai de paiement fournisseur ou avancer certains encaissements clients.'
}

export default function TensionDetection({ results }) {
  const tensionMonths = results.filter(r => r.enTension)
  const nearTension = results.filter(r => !r.enTension && r.soldeFin >= 0 && r.soldeFin < 20000)
    ...
    <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(15,35,64,0.08)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '20px 24px', maxHeight: '420px', overflowY: 'auto' }}>
        {tensionMonths.length === 0 && nearTension.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '32px 16px', background: '#f0fdf4', borderRadius: '12px' }}>
          ...
          <>
            {tensionMonths.map(r => (
              <div key={r.mois} style={{
                ...
                borderRadius: '10px', borderLeft: `4px solid ${r.critique ? '#dc2626' : '#ea580c'}`
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ fontWeight: '700', fontSize: '14px', color: '#0f2340' }}>{r.mois}</span>
                  <span style={{ fontSize: '13px', fontWeight: '700', color: r.critique ? '#dc2626' : '#ea580c' }}>
                ...
                <p style={{ fontSize: '12px', color: '#334155', fontStyle: 'italic', borderTop: '1px solid rgba(0,0,0,0.06)', paddingTop: '6px' }}>
                  ▶ {getTensionRecommendation(r)}
                </p>
              </div>
            ))}
            {nearTension.map(r => (
              <div key={r.mois} style={{
                marginBottom: '12px', padding: '14px 16px',
                background: '#fffbeb', borderRadius: '10px', borderLeft: '4px solid #d97706'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <span style={{ fontWeight: '700', fontSize: '14px', color: '#0f2340' }}>
                    {r.mois} <span style={{ fontWeight: '400', fontSize: '12px', color: '#d97706' }}>(vigilance)</span>
                  </span>
